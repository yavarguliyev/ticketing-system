...
export type CustomThrottlerOptions = { limit: number; ttl: number };

export const Throttle = (limit: number, ttl: number) => SetMetadata<string, CustomThrottlerOptions>(THROTTLER_LIMIT, { limit, ttl });
export const SkipThrottle = (skip = true) => SetMetadata<string, boolean>(THROTTLER_SKIP, skip);
