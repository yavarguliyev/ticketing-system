    ...
    while (true) {
      try {
        ...
        if (this.isOptimisticLockError(error) && currentRetry < retryOptions.maxRetries) {
          currentRetry++;
          this.logger.warn(
            `Optimistic concurrency conflict detected in ${context}. Retrying (${currentRetry}/${retryOptions.maxRetries})...`
          );
          ...
          throw new ConflictException(
            `Failed to complete ${context} after ${retryOptions.maxRetries} retries due to concurrent modifications`
          );
        } else {
          throw error;
        }
